...
| `scripts/generate-samples.ts` | Procedural sample generator (kick variants, perc, texture) |
...
- **Procedurally generated samples** (built by `scripts/generate-samples.ts`) are licensed `"Procedurally generated by psy-sampler build script — no copyright restriction"`.
